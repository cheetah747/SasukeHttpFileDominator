...
package com.sibyl.httpfiledominator.barcodescanneract
...
object PreferenceUtils {
    ...
    fun isAutoSearchEnabled(context: Context): Boolean =
        getBooleanPref(context, "pref_key_enable_auto_search", true)

    fun isMultipleObjectsMode(context: Context): Boolean =
        getBooleanPref(context, "pref_key_object_detector_enable_multiple_objects", false)

    fun isClassificationEnabled(context: Context): Boolean =
        getBooleanPref(context, "pref_key_object_detector_enable_classification", false)
    ...
    fun getConfirmationTimeMs(context: Context): Int = when {
        ...
        isAutoSearchEnabled(context) -> getIntPref(context, "pref_key_confirmation_time_in_auto_search", 1500)
        else -> getIntPref(context, "pref_key_confirmation_time_in_manual_search", 500)
    }
    ...
    fun getProgressToMeetBarcodeSizeRequirement(
        ...
        return if (getBooleanPref(context, "pref_key_enable_barcode_size_check", false)) {
            ...
            val requiredWidth = reticleBoxWidth * getIntPref(context, "pref_key_minimum_barcode_width", 50) / 100
    ...
    fun getBarcodeReticleBox(overlay: GraphicOverlay): RectF {
        ...
        val boxWidth = overlayWidth * getIntPref(context, "pref_key_barcode_reticle_width", 80) / 100
        val boxHeight = overlayHeight * getIntPref(context, "pref_key_barcode_reticle_height", 35) / 100
    ...
    fun getUserSpecifiedPreviewSize(context: Context): CameraSizePair? {
        return try {
            val previewSizePrefKey = "pref_key_rear_camera_preview_size"
            val pictureSizePrefKey = "pref_key_rear_camera_picture_size"
            val sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context)
            CameraSizePair(
    ...
    private fun getBooleanPref(context: Context, prefKey: String, defaultValue: Boolean): Boolean =
        PreferenceManager.getDefaultSharedPreferences(context).getBoolean(prefKey, defaultValue)
}
